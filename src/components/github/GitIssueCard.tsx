    ...
    return (
        ...
        >
            <summary className="cursor-pointer p-[2px] flex flex-wrap gap-2 items-center w-full box-border bg-neutral-100 dark:bg-neutral-800">
                ...
                <button
                    className="text-inherit text-center text-sm border-none w-8 h-8 p-[6px] rounded-lg bg-transparent hover:scale-105 focus:scale-105 transition box-border"
                    type="button"
                    onClick={() => {
                ...
                {mondayLinks &&
                    mondayLinks.map((link, index) => (
                        <a
                            ...
                            key={`link-${index}`}
                        >
                            Monday.com {link.slice(link.lastIndexOf('/') + 1)}{' '}
                            &rarr;
                        </a>
                    ))}
                {jiraLinks &&
                ...
                {sharepointLinks &&
                    sharepointLinks.map((link, index) => (
                        <a
                            ...
                            key={`link-${index}`}
                        >
                            SharePoint {link.slice(link.lastIndexOf('/') + 1)}{' '}
                            &rarr;
                        </a>
                    ))}
                {otherLinks &&
